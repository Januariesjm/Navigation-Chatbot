...
from model import parse_explore_rsp, parse_reflect_rsp, GPT2Model
...
if configs["MODEL"] == "GPT2":
    mllm = GPT2Model(model_name=configs["GPT2_MODEL"])
...
task_name = datetime.datetime.fromtimestamp(
    demo_timestamp).strftime("self_explore_%Y-%m-%d_%H-%M-%S")
...
print_with_color(
    "Please enter the description of the task you want me to complete in a few sentences:", "blue")
task_desc = input()
...
while round_count < configs["MAX_ROUNDS"]:
    ...
    prompt = re.sub(r"<task_description>", task_desc,
    ...
    if act_name == "tap":
        prompt = re.sub(r"<action>", "tapping",
                        prompts.self_explore_reflect_template)
    elif act_name == "text":
        continue
    elif act_name == "long_press":
        prompt = re.sub(r"<action>", "long pressing",
                        prompts.self_explore_reflect_template)
    elif act_name == "swipe":
        swipe_dir = res[2]
        if swipe_dir == "up" or swipe_dir == "down":
            ...
            act_name = "h_swipe"
        prompt = re.sub(r"<action>", "swiping",
                        prompts.self_explore_reflect_template)
    else:
        ...
        if decision == "INEFFECTIVE":
            ...
            if os.path.exists(doc_path):
                ...
                if doc_content[act_name]:
                    print_with_color(f"Documentation for the element {
                                     resource_id} already exists.", "yellow")
                    continue
            else:
                doc_content = {
                    "tap": "",
...
if task_complete:
    print_with_color(f"Autonomous exploration completed successfully. {
    ...
    print_with_color(f"Autonomous exploration finished unexpectedly. {
                     doc_count} docs generated.", "red")
